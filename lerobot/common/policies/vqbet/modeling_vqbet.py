...
class VQBeTPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        save_dir = Path("./before_model_normalize_when_select_action")
        os.makedirs(save_dir, exist_ok=True)
        # for key, value in batch.items():
        #     import re
        #     if re.fullmatch(r"observation\.image_\d+", key):
        #         img = value[0][0].detach().cpu().permute(1, 2, 0).numpy()  # [3, H, W] -> [H, W, 3]
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        import numpy as np
        from pathlib import Path
        save_dir = Path("./before_model_normalize_when_train")
        os.makedirs(save_dir, exist_ok=True)
        # for key, value in batch.items():
        #     import re
        #     if re.fullmatch(r"observation\.image_\d+", key):
...
class VQBeTRgbEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        os.makedirs(save_dir, exist_ok=True)
        img = x[0].detach().cpu().permute(1, 2, 0).numpy()  # [3, H, W] -> [H, W, 3]
        img = (img * 255).astype(np.uint8)
        fig, ax = plt.subplots(figsize=(4, 4))
        ax.imshow(img)
        plt.savefig(os.path.join(save_dir, f"cropped_image.png"))
