...
#         center_T_ref[:,0,3] = torch.tensor(-self.XMIN-vox_size_X/2.0, 
#                                   dtype=torch.float32, 
#                                   device=device)
#         center_T_ref[:,1,3] = torch.tensor(-self.YMIN-vox_size_Y/2.0, 
#                                   dtype=torch.float32, 
#                                   device=device)
#         center_T_ref[:,2,3] = torch.tensor(-self.ZMIN-vox_size_Z/2.0, 
#                                   dtype=torch.float32, 
#                                   device=device)
#         # scaling
...
#         mem_T_center[:,0,3] = torch.tensor(1./vox_size_X, 
#                                   dtype=torch.float32, 
#                                   device=device)
#         mem_T_center[:,1,3] = torch.tensor(1./vox_size_Y, 
#                                   dtype=torch.float32, 
#                                   device=device)
#         mem_T_center[:,2,3] = torch.tensor(1./vox_size_Z, 
#                                   dtype=torch.float32, 
#                                   device=device)
...
class Vox_util(object):
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        # (this makes the left edge of the leftmost voxel correspond to XMIN)
        center_T_ref = utils.geom.eye_4x4(B, device=device)
        center_T_ref[:,0,3] =torch.tensor((-self.XMIN-vox_size_X/2.0)).cuda()
        center_T_ref[:,1,3] = torch.tensor(-self.YMIN-vox_size_Y/2.0).cuda()
        center_T_ref[:,2,3] = torch.tensor(-self.ZMIN-vox_size_Z/2.0).cuda()

        # scaling
        # (this makes the right edge of the rightmost voxel correspond to XMAX)
        mem_T_center = utils.geom.eye_4x4(B, device=device)
        mem_T_center[:,0,0] = torch.tensor(1./vox_size_X).cuda()
        mem_T_center[:,1,1] = torch.tensor(1./vox_size_Y).cuda()
        mem_T_center[:,2,2] = torch.tensor(1./vox_size_Z).cuda()
        mem_T_ref = utils.basic.matmul2(mem_T_center, center_T_ref)
