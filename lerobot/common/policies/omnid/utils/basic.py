...
def pack_seqdim(tensor, B):  # [B, S, ...]->[B*S, ...]
...
def unpack_seqdim(tensor, B):  # [B*S, ...]->[B, S, ...]
    shapelist = list(tensor.shape)
    BS = shapelist[0]
    assert(BS%B==0)
...
def gridcloud3d(B, Z, Y, X, norm=False, device='cuda'):  # X,Y,Z=64,32,64 ,64*32*64=131072
    ...
    # these are B x N
    xyz = torch.stack([x, y, z], dim=2)  # (128,131072,3)
    # this is B x N x 3
    return xyz
