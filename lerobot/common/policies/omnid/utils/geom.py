import torch
import sys
# import diffusion_policy.model.utils.basic
import numpy as np
...
def safe_inverse(a): #parallel version  接受一个形状为 [B, 4, 4] 的张量 a，并返回其逆矩阵
    B, _, _ = list(a.shape)
    inv = a.clone()
...
def apply_4x4(RT, xyz):  # 将一个 4x4 的变换矩阵应用于一个点云，执行旋转和平移操作，并返回变换后的点云坐标
